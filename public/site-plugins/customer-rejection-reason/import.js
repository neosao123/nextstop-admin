 function Upload() {
    ...
    if (fileUpload.value !== '') {
        ...
        if (!allowedExtensions.test(file.name) || !mimeTypes.includes(file.type)) {
            ...
            fileUpload.value = '';
            $('#customerRejectionReasonList').addClass('d-none').hide();
            return;
        }

        if (typeof FileReader !== "undefined") {
            // Reset the table and progress bar before loading new file
            $('#customerRejectionReasonList').removeClass('d-none').show();
...
function validateExcelRows() {
    ...
    $.ajax({
        url: baseUrl + "/customer-rejection-reason/validate/excel",
        async: false,
        dataType: 'JSON',
        type: 'POST',
        data: {
...
$('#submitForm').on('submit', function(e){ 
		...
		$.ajax({ 
			...
			type:'POST',
			url: baseUrl + '/customer-rejection-reason/upload/excel',
			data: formData,
			contentType: false,
			cache: false,
			processData:false,	
			...
			success: function(response){
				...
				setTimeout(function() {
						window.location.href = baseUrl + '/customer-rejection-reason'; 
				}, 8000);
