...
function Upload() {
    ...
    if (fileUpload.value !== '') {
        ...
        if (!allowedExtensions.test(file.name) || !mimeTypes.includes(file.type)) {
            ...
            fileUpload.value = '';
            $('#goodstypeList').addClass('d-none').hide();
            return;
        }

        if (typeof FileReader !== "undefined") {
            // Reset the table and progress bar before loading new file
            $('#goodstypeList').removeClass('d-none').show();
...
function ProcessExcel(data) {
    ...
    table.border = "1";
    table.id = "tableID";
    table.setAttribute('class', "table table-bordered table-sm table-striped");

    // Insert header row
    var headers = [
        "Sr No", "Goods Type Name", "Status"
    ];
    ...
    for (var i = 0; i < excelRows.length; i++) {
        ...
        row.insertCell(-1).innerHTML = excelRows[i]["Good Type Name"] || "";
...
function validateExcelRows() {
    ...
    $.ajax({
        url: baseUrl + "/goods-type/validate/excel",
        async: false,
        dataType: 'JSON',
        type: 'POST',
        data: {
...
$('#submitForm').on('submit', function (e) {
    ...
    $.ajax({
        ...
        type: 'POST',
        url: baseUrl + '/goods-type/upload/excel',
        data: formData,
        contentType: false,
        cache: false,
        processData: false,
        ...
        success: function (response) {
            ...
            if (response.status == true) {
                ...
                setTimeout(function () {
                    window.location.href = baseUrl + '/goods-type';
                }, 8000);
