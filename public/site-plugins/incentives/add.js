$(document).ready(function () {
	...
	$('#incentive-save').on('click', function (e) {
	  if ($('#form-add-incentive').valid()) {
		let formData = new FormData($('#form-add-incentive')[0]);
		saveIncentive(formData);
	  }
	});

	function saveIncentive(formData) {
	  $.ajax({
		...
		dataType: 'json',
		beforeSend: function () {
		  $('#incentive-save').prop('disabled', true);
		},
		...
		complete: function () {
		  $('#incentive-save').prop('disabled', false);
		}
	  });
	}
