...
function Upload() {
    ...
    if (fileUpload.value !== '') {
        ...
        if (!allowedExtensions.test(file.name) || !mimeTypes.includes(file.type)) {
            ...
            fileUpload.value = '';
            $('#vehicleList').addClass('d-none').hide();
            return;
        }

        if (typeof FileReader !== "undefined") {
            // Reset the table and progress bar before loading new file
            $('#vehicleList').removeClass('d-none').show();
...
function ProcessExcel(data) {
    ...
    // Insert header row
    var headers = [
        "Sr No", "Vehicle Type", "Vehicle Name", "Vehicle Dimensions (L X B X H)", "Vehicle Maximum Load Capacity (kgs)", "Vehicle Per KM Delivery Charge", "Vehicle Per KM Extra Delivery Charge", "Vehicle Description", "Vehicle Rules", "Status"
    ];
    ...
    for (var i = 0; i < excelRows.length; i++) {
        ...
        row.insertCell(-1).innerHTML = excelRows[i]["Vehicle Type"] || "";
        row.insertCell(-1).innerHTML = excelRows[i]["Vehicle Name"] || "";
        row.insertCell(-1).innerHTML = excelRows[i]["Vehicle Dimensions (L X B X H)"] || "";
        row.insertCell(-1).innerHTML = excelRows[i]["Vehicle Maximum Load Capacity (kgs)"] || "";
        row.insertCell(-1).innerHTML = excelRows[i]["Vehicle Per KM Delivery Charge"] || "";
        row.insertCell(-1).innerHTML = excelRows[i]["Vehicle Per KM Extra Delivery Charge"] || "";
        row.insertCell(-1).innerHTML = excelRows[i]["Vehicle Description"] || "";
        row.insertCell(-1).innerHTML = excelRows[i]["Vehicle Rules"] || "";
...
function validateExcelRows() {
    ...
    $.ajax({
        url: baseUrl + "/vehicle/validate/excel",
        async: false,
        dataType: 'JSON',
        type: 'POST',
        data: {
        ...
        success: function (response) {
            ...
            if (response.msg === '') {
                ...
                $.each(invalidRows, function (index, val) {
                    $("table#tableID tr").eq(val + 1).css('background-color', 'transaparent');
                });
            }
        },
...
$('#submitForm').on('submit', function (e) {
    ...
    $.ajax({
        ...
        type: 'POST',
        url: baseUrl + '/vehicle/upload/excel',
        data: formData,
        contentType: false,
        cache: false,
        processData: false,
