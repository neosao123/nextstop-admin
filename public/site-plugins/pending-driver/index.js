$(document).ready(function () {
    ...
    // Select2 - Verification Status Type
    $('#search_verification_status_type').select2({
        placeholder: 'Select Type',
        allowClear: false,
        multiple: true
    })

    // Select2 - Verification Status
    $('#search_verification_status').select2({
    ...
    $('#search_filter').on('click', function () {
        ...
        // Check dependency
        if ((Array.isArray(search_verification_status_type) && search_verification_status_type.length > 0 && !search_verification_status) ||
            (search_verification_status && (!Array.isArray(search_verification_status_type) || search_verification_status_type.length === 0))) {
            toast("Please select both Verification Status Type and Verification Status, or leave both empty.", 'error');
        ...
        getDataTable(
            ...
            search_verification_status_type,
    ...
    $("#exportExcelBtn").on("click", function (e) {
        ...
        $.ajax({
            type: "GET",
            url: baseUrl + '/driver/pending/excel-export',
            ...
            success: function (response, status, xhr) {
                ...
                var filename = 'Pending-Partner' + '.csv';  // Default filename in case extraction fails
    ...
    $("#exportPdfBtn").on("click", function (e) {
        ...
        $.ajax({
            type: "get",
            url: baseUrl + "/driver/pending/pdf-export",
            data: {
                export: 1,
                _token: csrfToken,
                search_name: search_name ?? '',
            ...
            success: function (response) {
                ...
                link.download = 'Pending-Partner.pdf';
...
function getDataTable(search_name, search_driver_vehicle, search_driver_serviceable_location, search_account_status, search_verification_status_type, search_verification_status) {
    $.fn.DataTable.ext.errMode = "none";
    if ($.fn.DataTable.isDataTable("#dt-pending-driver")) {
        $("#dt-pending-driver").DataTable().clear().destroy();
    }
    var dataTable = $("#dt-pending-driver").DataTable({
        stateSave: false,
        ...
        paging: true,
        ajax: {
            url: baseUrl + "/driver/pending/list",
            type: "GET",
            data: {
                search_name: search_name ?? '',
                search_driver_vehicle: search_driver_vehicle ?? '',
...
function accountBlockOrUnblock(id, type) {
    ...
    $.confirm({
        ...
        buttons: {
            confirm: function () {
                $.ajax({
                    url: baseUrl + "/driver/pending/" + id + "/" + type,
                    type: "DELETE",
                    data: {
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            toast(response.error || "An error occurred while " + type + " the partner.", "error");
                        }
                    },
                    error: function (xhr) {
