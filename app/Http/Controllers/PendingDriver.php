...
class PendingDriver extends Controller
{
    /**
    ...
    {
        try {
            return view('pending-driver.index');
        } catch (\Exception $ex) {
            LogHelper::logError('An error occurred while the pending driver index page', $ex->getMessage(), __FUNCTION__, basename(__FILE__), __LINE__, __FILE__, '');
            // Return error response to the user
            return redirect()->back()->with('error', 'An error occurred while the pending driver  list.');
        }
    }
    ...
    public function vehicle(Request $request)
    {
        try {
            ...
            LogHelper::logError('An error occurred while the vehicle dropdown list - pending driver', $ex->getMessage(),  __FUNCTION__, basename(__FILE__), __LINE__, __FILE__, '');
            // Return error response to the user
    ...
    public function servicablelocation(Request $request)
    {
        try {
            ...
            LogHelper::logError('An error occurred while the servicable zone dropdown list - pending driver', $ex->getMessage(),  __FUNCTION__, basename(__FILE__), __LINE__, __FILE__, '');
            // Return error response to the user
            return response()->json([]);
        }
    }
    ...
    public function list(Request $request)
    {
        try {
            ...
            $filteredData = Driver::filterPendingDriver($search, $limit, $offset, $filterArray); // Assuming you have a method for filtering
            ...
            if ($result && $result->count() > 0) {
                foreach ($result as $row) {
                    ...
                    if ($showActions) {
                        ...
                        if (Auth::guard('admin')->user()->can('Pending-Driver.View-Edit')) {
                            $action .= '<a class="dropdown-item btn-view" href="' . url('driver/pending/' . $row->id) . '"> <i class="far fa-folder-open"></i> ' . __('index.view_or_edit') . '</a>';
                        }
                        if (Auth::guard('admin')->user()->can('Pending-Driver.Delete')) {
                            $action .= '<a class="dropdown-item btn-delete text-danger" data-id="' . $row->id . '"> <i class="far fa-trash-alt"></i> ' . __('index.delete') . '</a>';
                        }
                        if (Auth::guard('admin')->user()->can('Pending-Driver.Block')) {
            ...
            // Log the error
            LogHelper::logError('An error occurred while the pending driver list', $ex->getMessage(),  __FUNCTION__, basename(__FILE__), __LINE__, __FILE__, '');
            return response()->json([
                "message" => "An error occurred while fetching the pending driver list",
            ], 500);
        }
    }

    /**
     * exporting to excel a pending listing of the resource.
     * @author shreyasm@neosao
     */
    public function excelDownloadPendingDrivers(Request $request)
    {
        ...
        // Fetch filtered data
        $filteredData = Driver::filterPendingDriver($search, $limit, $offset, $filterArray);
        ...
        foreach ($records as $row) {
            ...
            $adminStatus = $this->showStatusForExcelExport($row->admin_verification_status);
        ...
        $csvFileName = 'Pending-Driver.csv';
    ...
    public function pdfDownloadPendingDriver(Request $request)
    {
        $search = $request->input('search.value') ?? "";
        $limit = $request->length ?? 0;
        $offset = $request->start ?? 0;
        ...
        $htmlContent .= '<table>';
        $htmlContent .= '<thead><tr><th>First Name</th><th>Last Name</th><th>Vehicle</th><th>Serviceable Zone</th><th>Phone</th><th>Email</th><th>Document Verification Status</th><th>Vehicle Verification Status</th><th>Training Verification Status</th><th>Account Status</th><th>Created Date</th></tr></thead>';
        $htmlContent .= '<tbody>';
    ...
    /**
     * The form for showing & editing the specified resource.
     * @author shreyasm@neosao
     */
    public function viewOrEdit(string $id)
    {
        try {
            $driver = Driver::where('id', $id)->with('serviceableZones')->where('is_delete', 0)->first();
            ...
            //dd($driver_training_video_deatils);
            if (!$driver) {
                // Log the error
                LogHelper::logError('An error occurred while view-or-edit the pending driver', 'The invalid driver',  __FUNCTION__, basename(__FILE__), __LINE__, __FILE__, $id);
                // Return error response to the user
                return redirect()->back()->with('error', 'The invalid driver.');
            }
            return view('pending-driver.view-or-edit', compact('driver', 'driver_vehicle_details', 'driver_personal_document', 'driver_bank_details', 'driver_vehicle_document_deatils', 'driver_training_video', 'driver_training_video_deatils'));
        } catch (\Exception $ex) {
            // Log the error
            LogHelper::logError('An error occurred while view-or-edit the pending driver', $ex->getMessage(),  __FUNCTION__, basename(__FILE__), __LINE__, __FILE__, '');
            // Return error response to the user
            return redirect()->back()->with('error', 'An error occurred while view-or-edit the pending driver.');
        }
    }

    /**
     * Verification of personal details & documents the specified resource.
     */
    public function verifyPersonalDetails(Request $request, string $id)
    {
        try {
            ...
            LogHelper::logSuccess('The pending driver personal details verified successfully.', __FUNCTION__, basename(__FILE__), __LINE__, __FILE__, $id);
            ...
            LogHelper::logError('An error occurred while verifying the pending driver personal details', $ex->getMessage(), __FUNCTION__, basename(__FILE__), __LINE__, __FILE__, $id);
            return redirect()->back()->with('error', 'An error occurred while verifying the driver personal details.');
        }
    }

    /**
     * Update the specified personal information & document resource in storage.
     * @author shreyasm@neosao
     */
        ...
        try {
            $validationRules =  [
                ...
                'driver_email' => [
                    ...
                    Rule::unique('drivers', 'driver_email')->ignore($id),
                ],
                'driver_phone' => [
                    'required',
                    'digits:10',
                    'integer',
                    Rule::unique('drivers', 'driver_phone')->ignore($id),
                ],
                'driver_gender' => [
                    'required',
            ...
            LogHelper::logSuccess('Partner updated successfully.', __FUNCTION__, basename(__FILE__), __LINE__, __FILE__, $id);
    ...
    /**
     * @author shreyasm@neosao
     * Remove the specified resource for driver from storage.
     */
    ...
    public function vehicle_information_update(Request $request, string $id)
    {
        ...
        try {
            ...
            return response()->json([
                'success' => false,
                'error' => 'An error occurred while updating the Partner vehicle details.'
            ]);
        }
    }
    ...
    public function vehicle_information_verify(Request $request, string $id)
    {
        try {
            ...
            $driver = Driver::find($id);
            if (!$driver) {
                return redirect()->back()->with('error', 'Partner vehicle not found.');
            }
            ...
            LogHelper::logSuccess('The Partner vehicle details verified successfully.', __FUNCTION__, basename(__FILE__), __LINE__, __FILE__, $id);
    ...
    {
        try {
            $driver = Driver::find($r->id);

            if (!$driver) {
                // Log the error
                LogHelper::logError('An error occurred while deleting the Partner photo', 'Invalid driver photo', __FUNCTION__, basename(__FILE__), __LINE__, __FILE__, $id);
                // Return JSON error response
                return response()->json(['error' => 'The Partner photo is invalid.'], 404);
            }
            ...
            // Log success
            LogHelper::logSuccess('The Partner photo deleted successfully', __FUNCTION__, basename(__FILE__), __LINE__, __FILE__, $id);
            // Return JSON success response
            return response()->json(['success' => 'The Partner photo deleted successfully']);
        } catch (\Exception $ex) {
            // Log the error
            LogHelper::logError('An error occurred while deleting the Partner photo', $ex->getMessage(), __FUNCTION__, basename(__FILE__), __LINE__, __FILE__, '');
            // Return JSON error response
            return response()->json(['error' => 'An error occurred while deleting the Partner photo.'], 500);
        }
    }
    ...
    {
        try {
            $driver_vehicle_document_details = DriverVehicleDocumentDetails::find($r->id);

            if (!$driver_vehicle_document_details) {
                // Log the error
                LogHelper::logError('An error occurred while deleting the vehicle document', 'Invalid vehicle document', __FUNCTION__, basename(__FILE__), __LINE__, __FILE__, $r->id);
                // Return JSON error response
    ...
    /**
     * Fetching the training video verify.
     * @author seemashelar@neosao
     */
    public function training_video_verify(Request $request, string $id)
    {
        try {
            ...
            } elseif ($request->training_video_verification_status == "reject") {
            ...
            LogHelper::logSuccess('The Partner training video details verified successfully.', __FUNCTION__, basename(__FILE__), __LINE__, __FILE__, $request->id);
            ...
            LogHelper::logError('An error occurred while verifying the Partner training video', $ex->getMessage(), __FUNCTION__, basename(__FILE__), __LINE__, __FILE__, $request->id);
            return redirect()->back()->with('error', 'An error occurred while verifying the Partner training video.');
        }
    }
