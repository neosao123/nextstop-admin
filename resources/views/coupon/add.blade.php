...
@section('content')
  <div class="d-flex mb-4 mt-1">
    ...
    <div class="col">
      <h5 class="mb-0 text-primary position-relative"><span class="bg-200 dark__bg-1100 pe-3">{{ __('index.add_coupon') }}</span><span
          class="border position-absolute top-50 translate-middle-y w-100 start-0 z-index--1"></span></h5>
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="{{ url('/dashboard') }}" class="text-decoration-none text-dark">{{ __('index.dashboard') }}</a></li>
          <li class="breadcrumb-item active" aria-current="page">{{ __('index.add_coupon') }}</li>
        </ol>
      </nav>
    </div>
  ...
  <div class="col-lg-12">
    <div class="card mb-3">
      <form id="form-add-coupon" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="card-header bg-light">
          <h5 class="mb-0">{{ __('index.add_coupon') }}</h5>
        </div>
        <div class="card-body">
          <div class="row">
            <!-- Coupon Code -->
            ...
            <div class="col-lg-4 col-md-6 col-sm-12 mb-3">
              ...
              <input type="number" step="0.01" class="form-control " name="coupon_amount_or_percentage" id="coupon_amount_or_percentage" value="{{ old('coupon_amount_or_percentage') }}" />
            </div>

            <!-- Coupon Cap Limit -->
            <div class="col-lg-4 col-md-6 col-sm-12 mb-3">
              <label class="form-label">{{ __('index.coupon_cap_limit') }} <span class="text-danger">*</span></label>
              <input type="number" class="form-control " name="coupon_cap_limit" id="coupon_cap_limit" value="{{ old('coupon_cap_limit') }}" step="0.01" />
            </div>

            <!-- Coupon Min Order Amount -->
            <div class="col-lg-4 col-md-6 col-sm-12 mb-3">
              <label class="form-label">{{ __('index.coupon_min_order_amount') }} <span class="text-danger">*</span></label>
              <input type="number" class="form-control " name="coupon_min_order_amount" id="coupon_min_order_amount" value="{{ old('coupon_min_order_limit') }}" step="0.01" />
            </div>
            ...
            <div class="col-lg-4 col-md-6 col-sm-12 mb-3">
              ...
              <input type="text" class="form-control " name="coupon_start_date" id="coupon_start_date" placeholder="dd/mm/yyyy" />
            </div>
			
			<!-- Coupon End Date -->
            <div class="col-lg-4 col-md-6 col-sm-12 mb-3">
              <label class="form-label">{{ __('index.coupon_end_date') }} <span class="text-danger">*</span></label>
              <input type="text" class="form-control " name="coupon_end_date" id="coupon_end_date" placeholder="dd/mm/yyyy" />
            </div>

            <!-- Coupon Description -->
            <div class="col-lg-12 col-md-12 col-sm-12 mb-3">
              <label class="form-label">{{ __('index.coupon_description') }}</label>
              <textarea class="tinymce" name="coupon_description" id="coupon_description">{{ old('coupon_description') }}</textarea>
            </div>
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 mb-3 position-relative">
              ...
              <input type="file" class="form-control" name="coupon_image" id="coupon_image" accept=".jpg, .jpeg, .png" />
        ...
        <div class="card-footer bg-light text-end">
          <button class="btn btn-primary" id="coupon-save" type="button">{{ __('index.save') }}</button>
        </div>
      </form>
...
@push('scripts')
  ...
  <script src="{{ asset('site-plugins/coupon/add.js?v=' . time()) }}"></script>
@endpush
