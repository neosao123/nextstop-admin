@extends('layout.master', ['pageTitle' => __('index.driver_payment_requests')])
@push('styles')
...
<div class="d-flex mb-4 mt-1" id="sel-projectId">
    ...
    <div class="col">
        <div class="">
            <h5 class="mb-0 text-primary position-relative"><span class="bg-200 dark__bg-1100 pe-3">{{__('index.driver_payment_requests')}}</span><span class="border position-absolute top-50 translate-middle-y w-100 start-0 z-index--1"></span></h5>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                     @if(Auth::guard('admin')->user()->can('Dashboard.View', 'admin'))
                    <li class="breadcrumb-item"><a href="{{ url('/dashboard') }}" class="text-decoration-none text-dark">{{__('index.dashboard')}}</a></li>
                    @endif
                    <li class="breadcrumb-item active" aria-current="page">{{__('index.driver_payment_requests')}}</li>
                </ol>
            </nav>
        </div>
    </div>
            ...
            <div class="card-body">
                <div class="table-responsive scrollbar">
                    <table id="dt-transaction" class="table table-hover">
                        <thead>
                            <tr>  
                               @if (Auth::guard('admin')->user()->canany(['Driver Earning.View', 'Driver Earning.Wallet-Transaction'])) 
      ...
      <div class="modal-body">
         <div class="row">			
			<div class="col-lg-12 col-md-12 col-sm-12 pb-2">
				 <label class="form-label">{{__('index.wallet_balance')}}   <small class="text-danger">* ....Minimum ₹ {{$miniBalance}} should always remain in the partner wallet</small></label>
...
<script>
 ...
 <script src="{{ asset('assets/vendors/select2/select2.min.js') }}"></script>
  ...
  <script src="{{ asset('site-plugins/driver-earning/index.js?v=' . time()) }}"></script>
@endpush