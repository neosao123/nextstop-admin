@php
  $pageTitle = __('index.view_incentives');
@endphp
...
@section('content')
  <div class="d-flex mb-4 mt-1">
    ...
    <div class="col">
      <h5 class="mb-0 text-primary position-relative"><span class="bg-200 dark__bg-1100 pe-3">{{ __('index.view_incentives') }}</span><span
          class="border position-absolute top-50 translate-middle-y w-100 start-0 z-index--1"></span></h5>
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="{{ url('/dashboard') }}" class="text-decoration-none text-dark">{{ __('index.dashboard') }}</a></li>
          <li class="breadcrumb-item active" aria-current="page">{{ __('index.view_incentives') }}</li>
        </ol>
      </nav>
    </div>
    ...
    <div class="card mb-3">
      <form >
        ...
        <div class="card-header bg-light">
          <h5 class="mb-0">{{ __('index.view_incentives') }}</h5>
        </div>
        <div class="card-body">
          <div class="row">
            ...
            <div class="col-lg-4 col-md-6 col-sm-12 mb-3">
              ...
              <input type="number" class="form-control " name="amount" id="amount" value="{{ $wallet->amount }}" step="0.01" readonly/>
            </div>

           <!-- Operation  -->
            <div class="col-lg-4 col-md-6 col-sm-12 mb-3">
			  <label class="form-label">{{ __('index.operation') }}</label>
			  <select class="custom-select form-control" name="operation" id="operation" readonly>
        ...
        <option value="sub" {{ $wallet->type == 'incentives deduction' ? 'selected' : '' }}>{{ __('Subtract') }}</option>
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 mb-3">
              ...
              <textarea class="form-control" name="reason" id="reason" readonly >{{ $wallet->message }}</textarea> 
            </div>
