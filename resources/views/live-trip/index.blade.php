@extends('layout.master', ['pageTitle' => __('index.live_trips')])
@push('styles')
...
<div class="d-flex mb-4 mt-1" id="sel-projectId">
    ...
    <div class="col">
        <div class="">
            <h5 class="mb-0 text-primary position-relative"><span class="bg-200 dark__bg-1100 pe-3">{{__('index.live_trips')}}</span><span class="border position-absolute top-50 translate-middle-y w-100 start-0 z-index--1"></span></h5>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    @if(Auth::guard('admin')->user()->can('Dashboard.View', 'admin'))
                    <li class="breadcrumb-item"><a href="{{ url('/dashboard') }}" class="text-decoration-none text-dark">{{__('index.dashboard')}}</a></li>
                    @endif
                    <li class="breadcrumb-item active" aria-current="page">{{__('index.live_trips')}}</li>
                </ol>
            </nav>
        </div>
    </div>
            ...
            <div class="card-body row">
				...
				<div class="col-lg-4 col-md-6 col-sm-12 mb-3">
				 ...
				 <div class="col-12 text-end  mt-3">
					...
					@if(Auth::guard('admin')->user()->can('Trip.Export', 'admin'))
            ...
            <div class="card-body">
                <div class="table-responsive scrollbar">
                    <table id="dt-trip" class="table table-bordered table-hover" style="width:100%">
                        <thead>
                            <tr>  
                                @if(Auth::guard('admin')->user()->canany(['Trip.View']))
                                ...
                                <th scope="col">{{__('index.pick_up_address')}}</th>
                                <th scope="col">{{__('index.drop_off_address')}}</th>
...
<script>
  ...
  <script src="{{ asset('site-plugins/live-trip/index.js?v=' . time()) }}"></script>
@endpush