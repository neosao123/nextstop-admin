@php
  $pageTitle = __('index.driver_rejection_reason');
@endphp
@extends('layout.master')
@push('styles')
  <style>
    .error {
      color: red;
    }
  </style>
@endpush
@section('content')
  <div class="d-flex mb-4 mt-1">
    <span class="fa-stack me-2 ms-n1">
      <i class="fas fa-circle fa-stack-2x text-300"></i>
      <i class="fa-inverse fa-stack-1x text-primary fas fa-film" data-fa-transform="shrink-2"></i>
    </span>
    <div class="col">
      <h5 class="mb-0 text-primary position-relative"><span class="bg-200 dark__bg-1100 pe-3">{{ __('index.driver_rejection_reason') }}</span><span
          class="border position-absolute top-50 translate-middle-y w-100 start-0 z-index--1"></span></h5>
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          @if (Auth::guard('admin')->user()->can('Dashboard.View', 'admin'))
            <li class="breadcrumb-item"><a href="{{ url('/dashboard') }}" class="text-decoration-none text-dark">{{ __('index.dashboard') }}</a></li>
          @endif
          @if (Auth::guard('admin')->user()->can('driver-Reason.List', 'admin'))
            <li class="breadcrumb-item"><a href="{{ url('/driver-rejection-reason') }}" class="text-decoration-none text-dark">{{ __('index.driver_rejection_reason') }}</a></li>
          @endif
          <li class="breadcrumb-item active" aria-current="page">{{ __('index.import_driver_rejection_reason') }}</li>
        </ol>
      </nav>
    </div>
    @if (Auth::guard('admin')->user()->can('driver-Reason.List', 'admin'))
      <div class="col-auto ms-2 align-items-center">
        <a class="btn btn-falcon-default btn-sm me-1 mb-1" href="{{ url('driver-rejection-reason') }}">
          <span class="px-2">{{ __('index.back') }}</span>
        </a>
      </div>
    @endif
  </div>
  <!-- Import driver Rejection Reason -->
  <div class="row gx-3">
    <div class="col-lg-12">
      <div class="card mb-3">
        <div class="card-header bg-light d-flex">
          <div class="col">
            <h5 class="mb-0">{{ __('index.import') }}</h5>
          </div>
          <div class="col-auto ms-2 align-items-center">
            <a class="btn btn-falcon-danger btn-sm me-1 mb-1" href="{{ asset('assets/templates/Template-Driver-Rejection-Reason.xlsx') }}" download="Template-Customer-Rejection-Reason.xlsx">
              <span class="px-2">{{ __('index.get_excel_template') }}</span>
            </a>
          </div>
        </div>
        <div class="card-body">
          <div class="text-danger">
            <p class="text-danger"><strong>{{ __('index.note') }}: </strong></p>
            <ul class="" type="">
              <li>
                <p class="mb-2">{{ __('index.make_sure_you_only_select_excel_template_and_please_make_sure_you_have_prepared_as_per_the_template_provided') }}</p>
              </li>
              <li>
                <p class="mb-2">{{ __('index.the_status_field_always_contains') }}<strong> active</strong>&nbsp{{ __('index.or') }}<strong> inactive </strong> {{ __('index.keyword') }}</p>
              </li>
            </ul>
          </div>
          <form action="{{ url('/driver-rejection-reason/upload/excel') }}" method="post" enctype="multipart/form-data" data-parsley-validate="" id="matform">
            @csrf
            <div class="row">
              <div class="col-md-4 form-group">
                <label>{{ __('index.select_excel_file') }}:</label>
                <div class="form-group">
                  <input type="file" class="form-control" name="uploadFile" id="uploadFile" required />
                </div>
              </div>
              <div class="col-md-6 form-group mt-2">
                <label></label>
                <div class="form-group">
                  <button class="btn btn-primary" type="button" id="upload" onclick="Upload()">{{ __('index.upload') }}</button>
                  <button class="btn btn-danger" type="button" onclick="window.location.reload();">{{ __('index.reset') }}</button>
                </div>
              </div>
            </div>
          </form>
        </div>
      </div>
      <div class="card d-none" id="DriverRejectionReasonList">
        <div class="card-header">
          <div class="row">
            <div class="col-sm-6">
              <h5 class="mb-0">{{ __('index.list') }}</h5>
            </div>
          </div>
        </div>
        <div class="row">
          <div class="col-sm-12">
            <div id="successMsg"></div>
            <form id="submitForm" method="post" name="s" enctype="multipart/form-data">

              <div class="card-body">
                <div id="message1" class="mb-3" style="display:none ">
                  <p class="text-danger"><strong>{{ __('index.the_following_rows_having_issues_please_check') }}</strong></p>
                  <p id="validMsgs"></p>
                </div>
                <div id="dvExcel">

                </div>
                <div class="progress mb-2" style="height:12px;">
                  <div id="file-progress-bar" class="progress-bar progress-bar-striped active"></div>
                </div>

                <button type="submit" name="submit" id="dataSubmit" class="btn btn-success">{{ __('index.submit') }}</button>

              </div>

            </form>
          </div>
        </div>
      </div>
    </div>
  </div>
@endsection
@push('scripts')
  <script>
    var baseUrl = "{{ url('/') }}"
  </script>
  <script type="text/javascript" src="{{ asset('assets/js/jszip.js') }}"></script>
   <script type="text/javascript" src="{{ asset('assets/js/xlsx.full.min.js') }}"></script>
  <script src="{{ asset('assets/js/jquery.validate.min.js') }}"></script>
  <script src="{{ asset('site-plugins/driver-rejection-reason/import.js?v=' . time()) }}"></script>
@endpush
